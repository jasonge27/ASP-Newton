...
sim_wc <- generate_sim_lognet(n=1000, d=5000, 0.2) 
}
...
cat("===========madelon==========\n")
prec = list(picasso=5.0*1e-6, ncvreg=1e-2, glmnet=5*1e-5, gcdnet=5*1e-5 )
test_lognet(madelon, prec)

cat("===========gisette==========\n")
prec = list(picasso=1.0*1e-4, ncvreg=1e-2, glmnet=5*1e-5, gcdnet=5*1e-5 )
...
cat("===========simwc==========\n")
prec = list(picasso=5*1e-2, ncvreg=1e-2, glmnet=5*1e-5, gcdnet=5*1e-5 )
test_lognet(sim_wc, prec)

cat("==========simic===========\n")
prec = list(picasso=1*1e-5, ncvreg=1e-3, glmnet=6*1e-5, gcdnet=3*1e-5 )
test_lognet(sim_ic, prec)
}

# Reproducing Figure 4
# Comparisions of Convergence Rate Measured in CPU Time.
#timing_lognet(sim_wc)
